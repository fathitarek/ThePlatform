...
return view('home.scheduled_posts',compact('userposts'));
    ...
    public function getaccesstoken1($page_id,Request $request){
        $accesstoken=  DB::table('app_users')
            ->join('app_users_profiles', 'app_users_profiles.app_user_id', '=', 'app_users.id')
            ->join('app_users_pages', 'app_users_pages.app_user_profile_id', '=', 'app_users_profiles.id')
            ->select('accessToken')
        ...
        //return $accesstoken ;
        //return $accesstoken->get(0)->accessToken ;
        return response()->json($accesstoken->get(0)->accessToken)->header('Access-Control-Allow-Origin', '*')->setCallback($request->input('callback'));
    }
...
AppUsersPosts::where('id', $id)->update(['publish' => 1, 'post_id' => $request->post_id , 'resource_id' => $request->resource_id]);
    ...
    public function updatepost1(Request $request,$id){
        AppUsersPosts::where('id', $id)->update(['publish' => 1, 'post_id' => $request->post_id , 'resource_id' => $request->resource_id]);
    ...
    public function cron(){
     ...
     public function cron2(){
      $url = "http://social.al-moasher.net/cron";
        ...
        curl_setopt($ch, CURLOPT_REFERER, 'facebook.com');
        $body = curl_exec($ch);
        echo $body;
        curl_close($ch);
...
if(count($checkdate)){
$minus=$checkdate-1;
$date->modify('+'.$minus.' week');  $finalschedule = $date->format('Y-m-d');
  $finalschedule.=' '.$scheduledpost;
}
}else{
...
/*    $category1 = DB::table('category_plan')
     //->join('app_users_profiles', 'app_users_profiles.app_user_id', '=', 'app_users.id')
     //->join('app_users_pages', 'app_users_pages.app_user_profile_id', '=', 'app_users_profiles.id')
     ->select(  'id', DB::raw(" DATE_ADD(concat('$lastpostdate',' ',`time`) , INTERVAL(doweek-1-DAYOFWEEK('$lastpostdate')) DAY) as schedule_date"))
...
echo $category1;*/
     $lastpostdate=date("Y-m-d",strtotime($lastpostdate));
    $category = DB::table('category_plan')
     //->join('app_users_profiles', 'app_users_profiles.app_user_id', '=', 'app_users.id')
     //->join('app_users_pages', 'app_users_pages.app_user_profile_id', '=', 'app_users_profiles.id')
     ->select(  'id', DB::raw(" DATE_ADD(concat('$lastpostdate',' ',`time`) , INTERVAL(doweek-1-DAYOFWEEK('$lastpostdate')) DAY) as schedule_date"))
     ->where([['category_id', '=', $category_id]])
 ...
 foreach($category as $categoryid){
    ...
    $ids[]=$categoryid->id;
    $schedule_dates[]=$categoryid->schedule_date;
 }

 if($lastplanid==0){
    $key=-1;
 }else{
   $key = array_search($lastplanid, $ids);
  // echo $key;
 }
...
if(isset($schedule_dates[$nextindex])){
$schedule_date=$schedule_dates[$nextindex];
//echo 'schedule_date : '.$schedule_date;
$planid=$ids[$nextindex];
echo $schedule_date;

}else{
  //  echo 'else stat(filename)';

$lastpostdate=date('Y-m-d H:i', strtotime($lastpostdate. ' +1 week'));
//echo 'lastpostdate : '.$lastpostdate;
goto start;
}
...
if(isset($schedule_date)){
                    $appUserPosts->created_time=$schedule_date;

                }else{
                $appUserPosts->created_time=($data['scheduleDateTime'])?$data['scheduleDateTime']:'';
 }if(isset($planid)){
      $appUserPosts->lastedit_by=$planid;
